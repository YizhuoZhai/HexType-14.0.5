		...
		const CXXRecordDecl *ParentDecl)
{
  ...
  //llvm::errs()<<"TargetStr = "<<TargetStr<<", TargetHashValue = "<<TargetHashValue <<"\n";
  //llvm::errs()<<"ParentStr = "<<ParentStr<<", ParentHashValue = "<<ParentHashValue <<"\n";
...
void CodeGenFunction::getTypeElement(const CXXRecordDecl *ClassDecl,
	...
	std::string DstTyStr = ClassDecl->getName().str();
...
llvm::Value *CodeGenFunction::EmitCXXNewExpr(const CXXNewExpr *E) {
  ...
  if(ClHandlePlacementNew && E->getNumPlacementArgs() >= 1) {
	  ...
	  if (ClassTy) {
		  ...
		  StringRef InstName("__placement_new_handle");

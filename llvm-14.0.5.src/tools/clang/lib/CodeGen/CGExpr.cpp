...
void CodeGenFunction::HexEmitCheck(StringRef FunName,
        ...
        //if (getContext().getSanitizerBlacklist().isBlacklistedType(EnabledHexTypeMask, OSR.str()))
    bool blacklisted = false;
    if (CGM.getContext().getNoSanitizeList().containsType(SanitizerKind::HexType, CurFn->getName())) {
      blacklisted = true;
    }

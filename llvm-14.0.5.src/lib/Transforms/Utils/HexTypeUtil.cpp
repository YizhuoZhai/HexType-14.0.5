  ...
  void HexTypeCommonUtil::updateCastingReleatedTypeIntoFile(Type *SrcTy) {
   if(SrcTy->isPointerTy()) {
      ...
      if(llvm::Type *AggTy = ptr->getElementType())
        if(AggTy->isStructTy()) {
		llvm::errs()<<"Inside updateCastingRelatedTypeInforFile : \n";
          char fileName[MAXLEN];
          char fileNameTmp[MAXLEN];
  ...
  void HexTypeLLVMUtil::removeNonCastingRelatedObj(StructElementInfoTy &Elements) {
    ...
    while (FindType && Elements.size() > 0) {
      ...
      for (auto &entry : Elements) {
        ...
        std::string TargetStr = entry.second->getName().str();
    ...
    if (!ResultCache) {
      ResultCache =
        new GlobalVariable(M,
                           ...
                           0,
                           "VerifyResultCache");
      ResultCache->setAlignment(llvm::Align(8));
    }
    ...
    if (!GObjTypeMap) {
      GObjTypeMap =
        new GlobalVariable(M,
                           ...
                           0,
                           "ObjTypeMap");
      GObjTypeMap->setAlignment(llvm::Align(8));
    }
      ...
      case CONOBJADD :
        {
          if (ClInlineOpt &&
            ...
            FunctionCallee initFunction =
              SrcM->getOrInsertFunction(TargetFn,
      ...
      case VLAOBJADD:
        {
          if (AllocType == REALLOC) {
            ...
            if (isFristEntry) {
              FunctionCallee initFunction =
                SrcM->getOrInsertFunction("__remove_oinfo",
                                                     VoidTy, IntptrTyN,
                                                     Int32Ty, Int64Ty,
                                                     Int32Ty, nullptr);
          ...
          FunctionCallee initFunction =
            SrcM->getOrInsertFunction("__update_oinfo",
                                                 VoidTy, IntptrTyN, Int64Ty,
                                                 Int32Ty, Int32Ty,

...
namespace {
  ...
  struct HexType : public ModulePass {
    ...
    void handleFnPrameter(Module &M, Function *F) {
      ...
      //llvm::errs()<<"MemcpyFunc : "<<*MemcpyFunc<<"\n";
      for (auto &a : F->args()) {
	  ...
	  //llvm::errs()<<"Inside handleFnPrameter : "<<MemcpyFunc->getName().str()<<"\n";
    ...
    void globalObjTracing(Module &M) {
      ...
      //std::tie(FGlobal, std::ignore) =
      //  createSanitizerCtorAndInitFunctions(M, "hextype.module_ctor",
      //                                      "__hextype_init", /*InitArgTypes=*/{},
      //                                      /*InitArgs=*/{}, "");
    ...
    virtual bool runOnModule(Module &M) {
      ...
      //llvm::errs()<<"emitTypeInfoAsGlobalVal(M)\n";
      ...
      //llvm::errs()<<"globalObjTracing(M) - begin\n";
      // Global object tracing
      globalObjTracing(M);
      //llvm::errs()<<"globalObjTracing(M) - end\n";
      // Stack object tracing
      stackObjTracing(M);
      //llvm::errs()<<"stackObjTracing(M);\n";
      return false;
    }
  };
}
